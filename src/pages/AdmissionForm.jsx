...
// The form is broken into these sections. Only one is shown at a time, and
// each section's own fields must pass validation before moving to the next.
const SECTIONS = [
    { title: 'Personal Information', fields: ['fullName', 'cnic', 'dob', 'gender', 'email', 'phone'] },
    { title: 'Academic Information', fields: ['qualification', 'institution', 'percentage'] },
    { title: 'Program Selection', fields: ['program', 'intake'] },
    { title: 'Address', fields: ['address', 'city', 'country'] },
    { title: 'Documents', fields: ['profilePhoto', 'cnicFile', 'transcript'] },
]
...
export default function AdmissionForm({ formData, onChange }) {
    ...
    // Runs the full validator but only keeps the errors that belong to one section
    function validateSection(index) {
        const allErrors = validateForm(formData)
        const sectionErrors = {}
        SECTIONS[index].fields.forEach((field) => {
            if (allErrors[field]) sectionErrors[field] = allErrors[field]
        })
    ...
    function handleSubmit(e) {
        ...
        if (Object.keys(foundErrors).length === 0) {
            ...
            // Jump back to the first section that still has an error
            const firstBadField = Object.keys(foundErrors)[0]
            const sectionIndex = SECTIONS.findIndex((s) => s.fields.includes(firstBadField))
            if (sectionIndex !== -1) setSection(sectionIndex)
            window.scrollTo({ top: 0, behavior: 'smooth' })
        }
    }
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-5">
                    <div className="mb-1.5 flex items-center justify-between text-xs font-medium text-slate-500">
                        <span>
                            Step {section + 1} of {SECTIONS.length}: {SECTIONS[section].title}
                        </span>
